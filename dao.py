...
def list_products():
    # Use a context manager for database connection and cursor
    with connect('products.db') as conn:
        cursor = conn.cursor()
        
        # Fetch all product rows from the database
        cursor.execute('SELECT * FROM products')
        products = cursor.fetchall()

    # Sort products if any exist
    return sorted(products, key=lambda x: x[0]) if products else []
